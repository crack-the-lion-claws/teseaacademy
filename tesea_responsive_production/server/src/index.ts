...
app.use(helmet({ contentSecurityPolicy: { directives: { defaultSrc: ["'self'"], scriptSrc: ["'self'"], styleSrc: ["'self'", "'unsafe-inline'"], imgSrc: ["'self'", 'data:', 'https:'], connectSrc: ["'self'", process.env.API_BASE_URL || "'self'"], frameAncestors: ["'none'"], objectSrc: ["'none'"] } }, crossOriginEmbedderPolicy: false }));
app.use(compression());
...
app.get('/assets/config.js', (_req, res) => {
  const publicConfig = { apiBaseUrl: process.env.API_BASE_URL || '', apiVersion: process.env.API_VERSION || 'v1', mode: process.env.API_MODE || (process.env.NODE_ENV === 'production' ? 'live' : 'mock'), oauth: { google: { clientId: process.env.GOOGLE_OAUTH_CLIENT_ID || '', redirectUri: process.env.GOOGLE_OAUTH_REDIRECT_URI || '' }, microsoft: { clientId: process.env.MICROSOFT_OAUTH_CLIENT_ID || '', tenant: process.env.MICROSOFT_OAUTH_TENANT || 'common', redirectUri: process.env.MICROSOFT_OAUTH_REDIRECT_URI || '' } } };
